...
def main():
    ...
    next_state  = None
    tick        = 0
    blink_phase = 0   # 0=없음 1=감는중 2=감김 3=뜨는중
    blink_t     = 0.0 # 0.0~1.0 진행도
    BLINK_CLOSE = 0.08  # 초: 눈 감는 시간
    BLINK_HOLD  = 0.04  # 초: 눈 감고 있는 시간
    BLINK_OPEN  = 0.10  # 초: 눈 뜨는 시간
    ANIM_FPS    = 60    # 전환 중 FPS
    ...
    while running:
        ...
        tick += 1
        # 전환 중엔 60fps, 평상시엔 10fps
        current_fps = ANIM_FPS if blink_phase > 0 else config.DISPLAY_FPS
        dt = 1.0 / current_fps
